...
use std::{
    hash::Hash, io::{Read, Write}, net::TcpListener, thread
};

use redis_starter_rust::{
    command::command::Command,
    commands_handlers::{ping, Icommand}, inMemory::db::db,
};
    ...
    for stream in listener.incoming() {
        thread::spawn(|| match stream {
            Ok(mut _stream) => {
                ...
                let ctx = db::new();
                ctx.set("a".to_owned(), "hello".to_owned());
                while let Ok(_) = _stream.read(&mut buffer) {
                    ...
                    let handler = command_object.handle(&mut _stream, ctx.clone());
                }
            }
            Err(_) => todo!(),
        });
