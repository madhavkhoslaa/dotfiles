...
import { Auth, isCoach } from "../middleware/Auth";
import { Roles } from "../types/roles";
const coachProgram = Router();

coachProgram.post("/meta-data", Auth, isCoach, async (req, res, next) => {
  try {
...
coachProgram.patch("/meta-data/:id", Auth, isCoach, async (req, res, next) => {
  try {
...
coachProgram.post("/exercise/", Auth, isCoach, async (req, res, next) => {
  try {
    console.log("Revieved Request for coaching creation", req.body);
    // Add validation and 400 Errors
...
coachProgram.patch("/exercise/:id", Auth, !isCoach, async (req, res, next) => {
  try {
    console.log("Revieved Request for coaching creation", req.body);
    // Add validation and 400 Errors
    let metricsInitialization: IExercise = req.body;
