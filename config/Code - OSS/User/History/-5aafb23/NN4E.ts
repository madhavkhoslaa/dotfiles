...
metricsRouter.patch('/:id', async(req, res, next)=>{
    try{
        let metricsInitialization: IAthleteMetrics = req.body;
        let updatedMetric = await Athletemetrics.updateOne({id: req.params.id}, metricsInitialization); 
        let response: IResponse<IAthleteMetrics> = {status: true, message: "metrics saves", data: updatedMetric}
        res.send(response).status(200)
    }
    catch(err){
        console.log(err)
