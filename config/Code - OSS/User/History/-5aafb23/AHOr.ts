...
metricsRouter.post("/", Auth, isConsumer, async (req, res, next) => {
  try {
    ...
    let metrics = new Athletemetrics({
...
metricsRouter.patch("/", Auth, isConsumer, async (req, res, next) => {
  try {
    let metricsInitialization: IAthleteMetrics = req.body;
    console.log(req.params, req.body);
    let updatedMetric = await Athletemetrics.findOneAndUpdate(
      { _id: req.body.authMiddleware.userId },
      metricsInitialization,
