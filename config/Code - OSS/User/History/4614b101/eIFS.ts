...
coachViewRouter.get("/profle/", Auth, isPublisher, async (req, res, next) => {
  let profile = await UserModel.findById(req.body.authMiddleware.userId);
  console.log(profile);
  res.send(profile);
});
...
// ToDo coachViewRouter.get("/views");
