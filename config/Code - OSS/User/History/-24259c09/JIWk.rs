...
fn string_to_20_bytes_binary(input: &str) -> String {
    // Create a Sha256 hash function
    let mut hasher = Sha1::new();
    // Update the hash function with the input string
    hasher.update(input);
        let hash = Sha1::digest(encoded_info);        // Get the hash result as a string
...
fn main() {
    ...
    if command == "decode" {
        ...
        let params = [
            ...
            ("port", "6881"),
            ("info_hash", &string_to_20_bytes_binary(&serde_bencode::to_string(&torrent.info).unwrap())),
            ("uploaded", "0"),
            ("downloaded", "0"),
