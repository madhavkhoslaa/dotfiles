...
fn url_encode(input: &str) -> String {
    ...
    for c in input.chars() {
        match c {
            // Unreserved characters (alphanumeric and '-', '_', '.', '~') remain unchanged
            'a'..='z' | 'A'..='Z' | '0'..='9' | '-' | '_' | '.' | '~' => {
                encoded.push(c);
            }
...
fn main() {
    ...
    if command == "decode" {
        ...
        let params = [
            ...
            ("port", "6881"),
            ("info_hash", &url_encode(&string_to_20_bytes_binary(serde_bencode::to_bytes(&torrent.info).unwrap()))),
            ("uploaded", "0"),
            ("downloaded", "0"),
            ("left", &torrent.info.peice_length.to_string()),
            ("compact", "1"),
        ]; 
        let url = format!("{}?{}", &torrent.announce, serde_urlencoded::to_string(params).unwrap());
