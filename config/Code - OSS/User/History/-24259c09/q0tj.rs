...
fn main() {
    ...
    if command == "decode" {
        ...
        let url = format!("{}?{}", &torrent.announce, serde_urlencoded::to_string(params))
        let url = reqwest::Url::parse_with_params(&torrent.announce + "&" + serde_urlencoded::to_string(params));
